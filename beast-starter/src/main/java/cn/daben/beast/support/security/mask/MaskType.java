...
package cn.daben.beast.support.security.mask;
...
public enum MaskType implements IMaskStrategy {
    ...
    EMAIL {
        @Override
        public String mask(String str, char character, int left, int right) {
            int index = str.indexOf(StringConst.AT);
            if (index <= 1) {
                return str;
            }
    ...
    BANK_CARD {
        ...
        public String mask(String str, char character, int left, int right) {
            ...
            for (int i = 0; i < midLength; ++i) {
                if (i % 4 == 0) {
                    buffer.append(StringConst.SPACE);
                }
                buffer.append(character);
            }
            buffer.append(StringConst.SPACE).append(cleanStr, length - endLength, length);
    ...
    IPV4 {
        @Override
        public String mask(String str, char character, int left, int right) {
            return CharSequenceUtil.subBefore(str, StringConst.DOT, false) + String
    ...
    IPV6 {
        @Override
        public String mask(String str, char character, int left, int right) {
            return CharSequenceUtil.subBefore(str, StringConst.COLON, false) + String
