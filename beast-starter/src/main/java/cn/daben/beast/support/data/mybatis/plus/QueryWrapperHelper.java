...
package cn.daben.beast.support.data.mybatis.plus;

import cn.daben.beast.exception.BadRequestException;
import cn.daben.beast.support.data.core.Query;
import cn.daben.beast.support.data.core.QueryIgnore;
import cn.daben.beast.support.data.core.QueryType;
...
public class QueryWrapperHelper {
    ...
    public static <Q, R> QueryWrapper<R> build(Q query) {
        ...
        // 获取查询条件中所有的字段
        List<Field> fieldList = ReflectKit.getNonStaticFields(query.getClass());
    ...
    private static <R> void parse(QueryType queryType,
        ...
        switch (queryType) {
            ...
            case BETWEEN -> {
                ...
                ValidationKit.throwIf(between.size() != 2, "[{}] 必须是一个范围", columnName);
            ...
            case IN -> {
                ValidationKit.throwIfEmpty(fieldValue, "[{}] 不能为空", columnName);
